/**
 * Deploy B2 Exam 1 Listening Part 11 (Cambridge Part 2): interview gap-fill + full audio (intro, 2 passes).
 *
 * Usage:
 *   node --loader ./scripts/alias-loader.mjs scripts/deploy-part11-exam1.mjs [examSlot] [--audio-only]
 */
    ...
    audioClips: [
      {
        ...
        storagePath: assembly.combinedStoragePath || `b2/exam-${examSlot}/part-11/full-v2.mp3`,
      },
    ],
  };
}
...
async function synthesizeInterviewPass(script, passIndex) {
  const rate = assembly.listeningTtsRate || process.env.LISTENING_TTS_RATE || '-14%';
  const voiceIndex = passIndex === 0 ? 4 : 6;
  console.error(`  Synthesizing interview pass ${passIndex + 1} (voices profile ${voiceIndex}, rate ${rate})…`);
  const result = await synthesizeListeningClipMp3(script, {
    extractIndex: voiceIndex,
    prosody: { rate },
  });
  if (!result?.base64) throw new Error('Interview TTS failed');
  const buf = Buffer.from(result.base64, 'base64');
  const sec = await getMp3DurationSec(buf);
  const minP = assembly.passDurationTargetSec?.min ?? 130;
  const maxP = assembly.passDurationTargetSec?.max ?? 175;
  const ok = sec >= minP && sec <= maxP;
  console.error(`    → ${formatDurationSec(sec)}${ok ? '' : ` [pass target ${minP}–${maxP}s]`}`);
  return buf;
}
...
const pass1 = await synthesizeInterviewPass(generated.script, 0);
const pass2 = await synthesizeInterviewPass(generated.script, 1);
...
const storagePath =
  assembly.combinedStoragePath || `b2/exam-${examSlot}/part-11/full-v2.mp3`;
...
const report = {
  ...
  wordCount: generated.script.split(/\s+/).length,
};
