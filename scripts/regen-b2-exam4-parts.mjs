/**
 * Regenera partes concretas del Examen 4 B2 con validación estricta.
 * Uso:
 *   node --loader ./scripts/alias-loader.mjs scripts/regen-b2-exam4-parts.mjs
 *   DRALO_REGEN_PARTS=3,4,5,6,7 node --loader ./scripts/alias-loader.mjs scripts/regen-b2-exam4-parts.mjs
 */
...
const EXAM_SLOT = 4;
const PARTS = (process.env.DRALO_REGEN_PARTS || '3,4,5,6,7')
  .split(',')
  .map((n) => Number(n.trim()))
...
console.log(
  `[regen] levelId=${level.id} slot=${EXAM_SLOT} parts=${PARTS.join(',')} useCodePrompts=${USE_CODE_PROMPTS}`,
);
  ...
  for (let outer = 0; outer < MAX_OUTER_RETRIES; outer += 1) {
    try {
      await generateAndPersistLevelExamPart(db, {
        ...
        // After first failure, fall back to code prompts (custom overrides often incomplete).
