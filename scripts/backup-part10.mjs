/**
 * Backup production B2 Exam 1 Part 10.
 * Usage: npx vercel env run --environment=production -- node scripts/backup-part10.mjs
 */
...
if (!parte?.id) {
  console.error('Parte 10 not found');
  process.exit(1);
}
...
const outPath = path.join(scriptsDir, 'generated', `backup-exam1-b2-part10-${Date.now()}.json`);
