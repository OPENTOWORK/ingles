-- =====================================================
-- SISTEMA COMPLETO PARA ENGLISH PRACTICE
-- Incluye: Roles + Teoría + Exámenes + Prueba Nivel + Training
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.temas_teoria (
    ...
    referencias_examen JSONB DEFAULT '[]'::jsonb, -- Qué exámenes/partes se refieren
...
CREATE TABLE IF NOT EXISTS public.tipos_examen (
    ...
    tipo TEXT NOT NULL, -- "full_exam", "use_of_english", "writing", "listening", "reading"
...
-- Secciones del examen (Reading Part 1, Use of English Part 3, etc.)
...
CREATE TABLE IF NOT EXISTS public.tareas_examen (
    ...
    orden INTEGER DEFAULT 0,
    notas TEXT,
    puntuacion_peso DECIMAL(3,2) DEFAULT 1.0, -- Peso de esta tarea en la sección
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.rubricas_examen (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    seccion_examen_id UUID REFERENCES public.secciones_examen(id) ON DELETE CASCADE,
    nombre_criterio TEXT NOT NULL, -- "Content", "Communicative Achievement", "Organisation", "Language"
    descripcion TEXT,
    puntuacion_maxima INTEGER DEFAULT 0,
    orden INTEGER DEFAULT 0,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
-- Puntuaciones de rúbricas (para Writing/Speaking)
...
CREATE TABLE IF NOT EXISTS public.sesiones_nivel (
    ...
    confianza_resultado DECIMAL(3,2) DEFAULT 0.00, -- 0.00 a 1.00
...
CREATE TABLE IF NOT EXISTS public.cursos (
    ...
    descripcion TEXT,
    enfoque TEXT, -- Descripción del enfoque
    nivel_partida TEXT, -- Nivel de partida requerido
...
CREATE TABLE IF NOT EXISTS public.lecciones (
    ...
    titulo TEXT NOT NULL,
    descripcion TEXT,
    explicacion TEXT, -- Explicación de la lección
...
-- Ejercicios dentro de cada lección (orden aleatorio desde biblioteca)
...
-- Intentos de entrenamiento (cada vez que hace un ejercicio)
CREATE TABLE IF NOT EXISTS public.intentos_entrenamiento (
    ...
    aprobo BOOLEAN DEFAULT FALSE, -- Si pasó el ejercicio
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Políticas básicas para usuarios y roles
...
-- Políticas para contenido público (teoría, cursos, etc.)
...
LEFT JOIN public.modelos_examen me ON e.id = me.examen_id
