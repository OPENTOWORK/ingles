...
const TABS = [
  ...
  { id: 'tareas', label: 'Tareas profesores' },
];

const TASK_ESTADO_OPTIONS = [
...
export default function CoordinatorPanel({ title = 'Panel de coordinador' }) {
  ...
  const [tasksTeacherFilter, setTasksTeacherFilter] = useState('');
  const [tasksEstadoFilter, setTasksEstadoFilter] = useState('');
  const [tasksLoading, setTasksLoading] = useState(false);
  const [tasksTablesReady, setTasksTablesReady] = useState(null);
  const [taskCreateProfesorId, setTaskCreateProfesorId] = useState('');
  const [taskFormStudents, setTaskFormStudents] = useState([]);
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      if (tasksTeacherFilter) params.set('profesorId', tasksTeacherFilter);
      if (tasksEstadoFilter) params.set('estado', tasksEstadoFilter);
      ...
      setTasks(data.tasks || []);
      setTasksSummary(
        data.summary || { total: 0, pending: 0, completed: 0, cancelled: 0 },
      );
      if (typeof data.tablesReady === 'boolean') setTasksTablesReady(data.tablesReady);
    } finally {
      setTasksLoading(false);
    }
  }, [tasksTeacherFilter, tasksEstadoFilter]);
  ...
  const loadTaskFormStudents = useCallback(async (profesorId) => {
    if (!profesorId) {
      setTaskFormStudents([]);
      return;
    }
    ...
    const data = await coordinatorFetch(`/api/coordinator/students?${params}`, {}, { soft: true });
    if (data.error) return;
    setTaskFormStudents(data.students || []);
  }, []);

  useEffect(() => {
    if (tab === 'tareas' && !loading) {
      loadTasks().catch(console.error);
    }
  }, [tab, loading, loadTasks]);

  useEffect(() => {
    if (tab === 'tareas') {
      loadTaskFormStudents(taskCreateProfesorId).catch(console.error);
    }
  }, [tab, taskCreateProfesorId, loadTaskFormStudents]);

  const handleCreateTask = async () => {
    if (!taskCreateProfesorId) {
      alert('Selecciona el profesor al que asignar la tarea.');
      return;
    }
    if (!taskTitle.trim()) {
    ...
    setSavingTask(true);
    try {
      await coordinatorFetch('/api/coordinator/tasks', {
        method: 'POST',
        body: JSON.stringify({
          action: 'create',
          profesorId: taskCreateProfesorId,
          titulo: taskTitle.trim(),
          descripcion: taskDesc.trim(),
          enlace: taskLink.trim(),
          fecha_limite: taskDue ? new Date(taskDue).toISOString() : null,
      ...
      setTaskDue('');
      setTaskAlumnoId('');
      if (!tasksTeacherFilter) setTasksTeacherFilter(taskCreateProfesorId);
      await loadTasks();
  ...
  const handleTaskEstadoChange = async (task, nuevoEstado) => {
    if (!task?.id || !task?.profesor_id) return;
    setSaving(true);
    try {
      await coordinatorFetch('/api/coordinator/tasks', {
        method: 'POST',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {tab === 'tareas' && (
          <div className="bg-white rounded-lg shadow p-6 space-y-6">
            {tasksTablesReady === false && (
              <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
                <p className="font-semibold">Tabla de tareas no disponible</p>
                <p className="mt-1">
                  Ejecuta{' '}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard label="Total tareas" value={tasksSummary.total} />
              <StatCard label="Pendientes" value={tasksSummary.pending} />
              <StatCard label="Completadas" value={tasksSummary.completed} />
              <StatCard label="Canceladas" value={tasksSummary.cancelled} />
            </div>
            ...
            <section className="border rounded-lg p-4 bg-gray-50 space-y-3">
              ...
              <p className="text-xs text-gray-600">
                Crea una tarea para un profesor. Si eliges alumno, debe estar asignado a ese
                profesor; si no, la verán todos sus alumnos.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={taskCreateProfesorId}
                    onChange={(e) => {
                      setTaskCreateProfesorId(e.target.value);
                      setTaskAlumnoId('');
                    }}
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={!taskCreateProfesorId}
                    className="border rounded px-3 py-2 text-sm w-full disabled:opacity-50"
                  >
                    <option value="">Todos los alumnos del profesor</option>
                    {taskFormStudents.map((s) => (
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="Ej. Practica Reading Part 2"
                    className="border rounded px-3 py-2 text-sm w-full"
                  />
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Detalle de la tarea para el alumno"
                    rows={3}
                    className="border rounded px-3 py-2 text-sm w-full"
                  />
                </div>
              ...
              <button
                ...
                disabled={savingTask || !taskCreateProfesorId}
                className="px-4 py-2 bg-violet-600 text-white rounded text-sm font-medium disabled:opacity-50"
              >
            ...
            <div className="flex flex-wrap gap-4 items-end">
              <div>
                ...
                <select
                  value={tasksTeacherFilter}
                  onChange={(e) => {
                    setTasksTeacherFilter(e.target.value);
                    if (e.target.value && !taskCreateProfesorId) {
                      setTaskCreateProfesorId(e.target.value);
                    }
                  }}
              ...
              <div>
                ...
                <select
                  value={tasksEstadoFilter}
                  onChange={(e) => setTasksEstadoFilter(e.target.value)}
                  className="border rounded px-3 py-2 text-sm min-w-[180px]"
                >
                  {TASK_ESTADO_OPTIONS.map((opt) => (
                  ...
                  {tasks.map((t) => (
                    <tr key={t.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">
                        {t.profesor?.nombre || t.profesor?.email || '—'}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        {t.alumno?.nombre || t.alumno?.email || 'Todos'}
                      </td>
                      <td className="px-4 py-3">
                        <select
                          value={t.estado}
                          disabled={saving}
                          onChange={(e) => handleTaskEstadoChange(t, e.target.value)}
