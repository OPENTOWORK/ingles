    ...
    <article className={styles.panel}>
      <header className={styles.header}>
        ...
        <p className={styles.meta}>
          Nivel del placement: <strong>{plan.placement_level || '—'}</strong>
          {plan.completed_at && (
            <>
              {' '}
              · Completada el {formatDateTime(plan.completed_at)}
            </>
          )}
        </p>
        {forAdmin && (
        ...
        <div className={styles.row}>
          ...
          <dd>
            {strengths.length > 0 ? (
              ...
              <span className={styles.muted}>Sin especificar</span>
            )}
          </dd>
        </div>
