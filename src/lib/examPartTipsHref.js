/**
 * Helpers de URL para tips de Exam theory (sin dependencias del catálogo de part-info).
 */
...
export function buildNivelesPartTipsHref(levelSlug, skillFolder, partNum) {
  ...
  if (level === 'a2' && (skill === 'listening' || skill === 'speaking')) {
    return `/niveles/${level}/${skill}/${n}`;
  }
  return `/niveles/${level}/${skill}/part-${n}`;
}
...
export function buildExamTheoryPartTipsHref(nivelesHref) {
  ...
  return `/teoria/exam-part-tips/${parsed.levelSlug}/${skill}/${partSeg}`;
}
