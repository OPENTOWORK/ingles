...
function livesError(error) {
  console.warn('[training/lives]', error?.code || '', error?.message || error);
  return NextResponse.json({ error: 'LIVES_UNAVAILABLE' }, { status: 503, headers: noStore });
}

/** Vidas de Training del alumno (gratis: 3, se recupera 1 cada 10 h). */
export async function GET(req) {
  const auth = await contextFor(req);
  if (auth.error) return auth.error;
  try {
    const status = await readTrainingLives(auth.userId, auth.ctx);
...
/** Gasta una vida al fallar una pregunta. */
export async function POST(req) {
  ...
  try {
    const status = await spendTrainingLife(auth.userId, auth.ctx);
    return NextResponse.json(status, { headers: noStore });
  } catch (error) {
    return livesError(error);
  }
