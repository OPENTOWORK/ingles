...
const levels = [
  ...
  { id: "intermedio", label: "🟡 Intermediate", description: "Simple structures" },
  { id: "avanzado", label: "🔵 Advanced", description: "Comprehension challenges" },
];

export default function A1UseOfEnglishPage() {
  return (
    <main
      style={{
        padding: "2rem",
    ...
    >
      <h1 style={{ fontSize: "2rem", marginBottom: "0.5rem" }}>🎓 Level A1</h1>
      ...
      >
        {levels.map(({ id, label, description }) => (
          <Link
            key={id}
            href={`/training/a1/use-of-english/${id}`} // ✅ corrected route
            style={{
              backgroundColor: "#fff",
              padding: "1.5rem",
              borderRadius: "12px",
